...
SketchpadCanvas.prototype.registerEvent = function(name, callback, optDescription) {
    var sketchpad = this.sketchpad
    this.canvas.addEventListener(name, callback, false)
}

SketchpadCanvas.prototype.initEvents = function() {
    var self = this
...
SketchpadCanvas.prototype.makeConstraintListViewInstanceList = function(alist, width, height) {
	...
	var cb1 = self.makeDOMElement('button', {name: c.__toString, fns: {onclick: function() { self.toggleCodeEditMode(false); self.inspectState(c) }, onmouseenter: function() { self.highlightPropertyThingsOf(c) }, onmouseleave: function() { self.clearSelections(true) }}, style: {background: '#d8cef6', width: width, height: height}})
	var cb2 = self.makeDOMElement('input', {fns: {onclick: function(event) { event.stopPropagation(); var aC = sketchpad.disabledConstraints[c.__id]; if (aC) { this.checked = true; self.addNewConstraint(c, true); delete sketchpad.disabledConstraints[c.__id] } else { this.checked = false; self.removeConstraint(c, true)}}}, style: {background: '#bcf5a9', height: 20}}, cb1, undefined, 'checkbox', {checked: !sketchpad.disabledConstraints[c.__id]})
...
SketchpadCanvas.prototype.makeConstraintListView = function() {
	...
	for (var tp in tree) {
		...
		var b1 = this.makeDOMElement('button', {name: shortTp, fns: {onclick: fn1(dscr, first, tp)}, style: {background: '#c0c0c0', width: width, height: height}})
		var b2 = this.makeDOMElement('label', {name: count, fns: {onclick: fn2(list, tp)}, style: {background: '#f5a9a9', height: 20}}, b1)
	    }
	}
...
SketchpadCanvas.prototype.inspectState = function(thing) {
	...
	props.push({name: p, type: defined ? pVal.__type : undefined, shortType: defined ? pVal.__shortType : undefined, value: pVal, expr: defined ? self.unparseJS(pVal, true, false) : undefined, hiddenExpr: defined ? self.unparseJS(pVal, false, false) : undefined})
    })
    var newPos
...
SketchpadCanvas.prototype.highlightPropertyThingsOf = function(thing) {  
    this.secondarySelections = getSketchpadThingProperties(thing, true)
    this.redraw()
}
...
SketchpadCanvas.prototype.getThingsOfType = function(type) {
    var res = []
    this.things.forEach(function(t) { if (t.__type === type) res.push(t) })
    return res
}
...
function SketchpadTile(name, inputs, ownRun, buttonsInfo, inspectorOfObj, fixedInputs, optDraw, optPosition, optBox) {
	...
	var inputShortType = inputInfo.shortType
	var inputValue = inputInfo.value
	var inputLabel = (inputShortType ? (inputShortType + ' ') : '') + inputName
	...
	if (inspectorOfObj) {
		...
		var propPointerLinePt = new Point(inputCoord.x, inputCoord.y)
    ...
    buttonsInfo.unshift({name: 'Class', fns: {onclick: viewClassFn}, style: {width: 40, background: 'pink'}})
    buttonsInfo.unshift({name: 'X', fns: {onclick: removeFn}, style: {background: '#ff6666'}})
...
function getSketchpadThingProperties(obj, getValues) {
    var res = []
    var keys = Object.keys(obj)
    for (var i = 0; i < keys.length; i++) {
	var p = keys[i]

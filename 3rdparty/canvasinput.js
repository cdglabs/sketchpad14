...
(function() {
	...
	self._x = o.x || 0;
	self._y = o.y || 0;
	self._origX = self._x; //HS
	self._origY = self._y; //HS
	...
	//HS
	draw: function(canvas, origin) {
	    this._x = this._origX + origin.x
	    this._y = this._origY + origin.y
	    this.render()
	},
	...
	render: function() {
	    var self = this,
            ...
            sw = self.shadowW,
            sh = self.shadowH,	    
	    shadowL = self.shadowL,
	    shadowT = self.shadowT;
		...
		self._roundedRect(ctx, shadowL, shadowT, w - sw, h - sh, br);
		ctx.fill();
		...
		// draw the selection
		var paddingBorder = self._padding + self._borderWidth + shadowT;
		...
		// draw the text
		var textX = self._padding + self._borderWidth + shadowL,
		...
		if (isBlur > 0) {
		    ...
		    self._roundedRect(ctx, bw + shadowL, bw + shadowT, w - bw * 2 - sw, h - bw * 2 - sh, br);
		    ctx.clip();

		    // draw the inner-shadow from the off-DOM canvas
		    ctx.drawImage(self._shadowCanvas, 0, 0, scw, sch, bw + shadowL, bw + shadowT, scw, sch);
		}
